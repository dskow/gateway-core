...
> **Status:** Pattern documentation. Partially implemented in `internal/envelope/` (autonomous-safe default; constraints, bounds, dampener, and shadow stages built; multi-agent pipeline skeleton built). Full pipeline scheduled for the agentic phase of the [2030 plan](API_GATEWAY_MAIN_PLAN_2030.md).
...
control plane. They must never put the control plane into a state that is
unsafe, unstable, or outside operator-defined bounds — and the control plane
must never become dependent on the agents to function.
...
Use this pattern when **all** the following hold:
...
| Shadow simulator (traffic replay)                                         | **Built (skeleton + contract)** — `Simulator` interface, `ShadowVerdict`, `ShadowRegistry` with per-Kind opt-in (`Enable`/`Disable`) and a hard per-evaluation timeout, wired as the fourth pipeline stage via `WithShadow`. A simulator-detected regression short-circuits with stage `"shadow"`, `DecisionReject`, and a structured `*ShadowOutcome` whose `Detail` lists the regressed SLO dimensions. A timeout short-circuits with stage `"shadow"`, `DecisionDefer`, and `RetryAfter` set to the configured timeout. A simulator-internal error short-circuits with stage `"shadow"` and `DecisionReject` (reason `simulator_internal_error`) so a buggy simulator never silently green-lights a proposal. Ships `NoopSimulator` (autonomous-safe pass-through) and `DefaultSLOScorer` (latency-P99 + error-rate baseline comparison). A real production replay engine — capturing a window of recent traffic and exercising the proposed configuration in a sandboxed copy of the control plane — remains designed but not built; the contract above is what it must satisfy. | `internal/envelope/shadow.go`                                         |
| Multi-agent pipeline (Planner / Verifier / Safety / Observer)             | **Built (skeleton + contract)** — `Role` enum (`RolePlanner` / `RoleVerifier` / `RoleSafety` / `RoleObserver`), `Planner` and `Reviewer` interfaces, `PlanResult` / `ReviewResult` / `PipelineOutcome` value types, and a `Pipeline` runner that enforces non-decreasing-Role order at construction, runs exactly one Planner per cycle, short-circuits on the first vetoing Reviewer, and self-disables after a configurable streak of agent errors via `ErrorBudget`. Operator override is exposed through `Disable` / `Enable` plus an `Enabled() bool` query (`PipelineOutcome.EnabledFrom` carries the wall-clock time the next Run is allowed when the auto-disable cooldown is in effect). Outcomes are stable codes: `OutcomeEmit` (proposal authored and reviewed), `OutcomeNoProposal` (planner declined), `OutcomeVeto` (reviewer rejected with structured reason), `OutcomeError` (agent returned an error; counts toward the budget), `OutcomeDisabled` (operator-disabled or budget-exhausted). `PlannerFunc` and `ReviewerFunc` adapters are provided for tests and trivial agents. The Pipeline produces proposals; the agentic sidecar (out of scope of this package) is the caller that hands them to `Envelope.Submit`. Concrete agent implementations (LLM-backed planners, statistical verifiers, etc.) and the sidecar that drives Pipeline.Run on a schedule remain designed but not built; the contract above is what they must satisfy. | `internal/envelope/agent.go`                                          |
...
constraint, a bound, the dampener, or the shadow simulator still falls
through to a fallback rejection. This matches the pattern's central
claim — the deterministic core works without agents, and agents are
added incrementally without ever putting the data path at risk. The
constraints stage gives clearer rejection reasons for malformed or
unconstitutional proposals (e.g., a non-positive rate-limit value is
rejected at stage `constraints` with reason
...
`bounds` with a reason like `bounds(rate_limit): above_maximum`; the
dampener stage defers proposals that arrive inside a cooldown window
...
hysteresis band (`dampener(rate_limit): below_hysteresis`); the shadow
stage rejects proposals whose simulated replay regresses an SLO
(`shadow(rate_limit): regression (latency_p99(+30ms))`) and defers
proposals whose simulator did not return inside the hard timeout
(`shadow(rate_limit): timeout`). None of the four stages weakens the
autonomous-safe contract, because nothing in the deterministic core
ever observes the rejection. The agent pipeline is upstream of all
four stages and is itself optional: a `Pipeline` produces proposals
that the sidecar submits to `Envelope.Submit`, but a gateway with
no Pipeline at all is the autonomous-safe default and is exactly
how the gateway runs today.
